...
public class RCodeParser {
    ...
    // Index for the WORDS
    final static int  IVR=0;
    final static int  IGF=1;
    final static int  IX=2;
    final static int  IY=3;
    final static int  IR=4;
    final static int  IF=5;    // Feed
    final static int  IZ=6;   // Slider position
    final static int  IS=7;   // Slider speed
    final static int  IP=8;  // Conditional continue
    final static int  IQ=9;  // Conditional stop
    final static int  IM=10;  // System function
    final static int  IG1=11;  // Gripper = 1
    final static int  IG2=12;  // Handler = 2
    final static int  IG3=13;  // Transfer = 3
    final static int  IG4=14;  // Not defined yet
    final static int  IG5=15;  // Not defined yet
    final static int  IG6=16;  // Not defined yet
    final static int  IG7=17;  // Transfer = 3
    final static int  IG8=18;  // Not defined yet
    final static int  IG9=19;  // Not defined yet
    final static int  ISB=20;  // Skip Symbol
    final static int  ISK=21;  // Skip command word SK
    ...
    // Error code
    static public RCodeErrors rcError = RCodeErrors.rcNoError;
    static int exeSts=0;
    ...
    enum RCPrgmStatus {rcNotLoaded, rcLoaded, rcRuning, rcHolding, rcEnded};
    // Status of the RCode program
    static RCPrgmStatus rcStatus;
    ...
    RCPrgmStatus getStatus(){ return rcStatus; }
    ...
    public RCodeParser()
    {
        ...
        rcStatus = RCPrgmStatus.rcNotLoaded;
        execBlock = 0;
    }
    ...
    public boolean loadFile()
    {
        ...
        String crtBlock;
        if(  rcStatus != RCPrgmStatus.rcNotLoaded )
        {
            exeSts = 1;
            return false;
        }
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(rcPrgmFile)))
        {
            ...
            rcStatus = RCPrgmStatus.rcLoaded;
            execBlock = 0;
            exeSts = 500;
        } catch (Exception ex) {
        ...
        if(blockPart.matches(".*[\\#$].*"))
        {
            ...
            if( blockPart.contains("#") )
            {
                ...
                switch( tmpPart )
                {
                    ...
                    break;
                    case "G": // G allready in good format as G1
                        break;
    ...
    private boolean preParseVarDeclaration(String blockPart )
    {
        ...
        */
        ...
        if( !( blockPart.startsWith( wLibrary[IVR]) || blockPart.startsWith( wLibrary[IGF])) ) return false;
        // It's a variable or G function declaration
        varDecl =  ( blockPart.startsWith( wLibrary[IVR]) );
        blockPart = cleanUpBlock(blockPart);
...
//                parts = blockPart.split(regex);
    ...
    //  Indexes:    {IX,IY,IR,IF IZ IS IP IQ IM IGC IGT IGE IVR}
    //  rcWord      [25,50,10,0 ,0 ,0 ,0 ,0 ,0 ,0  ,20 ,0  ,0  ]

    public boolean  nextBlock()
    {
        String [] rcWords = new String[WORDS_COUNT];
        // If the program is loaded and running we can parse the next BLOCK
        if(( rcStatus == RCPrgmStatus.rcLoaded || rcStatus == RCPrgmStatus.rcRuning ) &&  execBlock < noBlocks )
        {
            exeSts = 6;
        ...
        {
            execBlock=0; rcStatus = RCPrgmStatus.rcEnded;
        }
        if( rcSetPoints.get("M99") != null && rcSetPoints.get("M99").equals("99") )
            rcStatus = RCPrgmStatus.rcEnded;
        return true;
    }
